...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        values = [self.getQValue(state, i) for i in legal_actions]
        if not len(values):
            return 0
        return max(values)
    ...
    def computeActionFromQValues(self, state):
        ...
        listactions = [action for action in legal_actions if self.getQValue(state, action) == max]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for f, v in feat.items():
            self.weights[f] = self.weights[f] + self.alpha * (diff * v)
